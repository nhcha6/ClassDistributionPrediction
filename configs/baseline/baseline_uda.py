...
# run for longer to assess supervised training
samples_per_gpu = 16
workers_per_gpu = 1
# total_iter = int(total_num / (samples_per_gpu * gpu))
total_iter = total_num
# test and save every 100 iterations
test_interval = 2000
save_interval = 1000
# # -------------------------dataset------------------------------
img_norm_cfg = dict(
...
if dataset_name in ['D2N', 'C2B', 'C2N']:
    classes = ('truck', 'car', 'rider', 'person', 'train', 'motorcycle', 'bicycle', 'bus')
elif dataset_name[0:3] in ['OAK', 'V2O']:
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + f'unlabeled_data.json',
        img_prefix=data_root + 'unlabeled_data/',
        classes=classes,
